...
Console.WriteLine($"\nWrite the 1000 largest orders to: {fname("Kata9.json")}");
using (Stream s = File.Create(fname("Kata9.json")))
using (TextWriter writer = new StreamWriter(s))
    writer.Write(JsonSerializer.Serialize<List<Order>>(largestOrders, new JsonSerializerOptions() { WriteIndented = true }));

Console.WriteLine($"\nWrite the 1000 largest orders to: {fname("Kata9.xml")}");
var xs = new XmlSerializer(typeof(List<Order>));
using (Stream s = File.Create(fname("Kata9.xml")))
